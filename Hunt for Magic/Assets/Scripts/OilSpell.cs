...
public class OilSpell : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (enemy != null && enemy.tag == "Monster")
        {
            ...
            {
                Instantiate(_oilPool, new Vector3(other.transform.position.x, 0.13f, other.transform.position.z), Quaternion.Euler(0, 0, 0));
            }
        ...
        {
            Instantiate(_oilPool, new Vector3(transform.position.x, 0.13f, transform.position.z), Quaternion.Euler(0, 0, 0));
            Destroy(GetComponentInParent<Transform>().gameObject);
        }
